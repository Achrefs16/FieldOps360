...
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault secrets enable -path=secret kv-v2 || true"
...
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault kv put secret/postgresql password='${DB_PASS}'"
echo "PostgreSQL secret written"

# 2. Redis Secret
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault kv put secret/redis password='${REDIS_PASS}'"
echo "Redis secret written"

# 3. MinIO Secret
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault kv put secret/minio access_key='${MINIO_AK}' secret_key='${MINIO_SK}'"
echo "MinIO secret written"

# 4. SMTP Credentials
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault kv put secret/fieldops/smtp smtp_user='${SMTP_USER}' smtp_pass='${SMTP_PASS}'"
echo "SMTP secret written"

echo "[4/5] Enabling Kubernetes Auth method..."
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault auth enable kubernetes || true"

# Configure K8s auth
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault write auth/kubernetes/config kubernetes_host=https://kubernetes.default.svc.cluster.local"

echo "[5/5] Creating Vault policies and roles for K8s Service Accounts..."

# Policy allows reading any secret
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault policy write read-all - <<EOF
path \"secret/data/*\" {
  capabilities = [\"read\"]
}
EOF"
...
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault write auth/kubernetes/role/auth-service bound_service_account_names=auth-service bound_service_account_namespaces=fieldops-auth policies=read-all ttl=1h"

# Role for backup cronjobs in fieldops-data
${KUBECTL_CMD} exec -n ${VAULT_NAMESPACE} ${VAULT_POD} -- sh -c "vault write auth/kubernetes/role/backup-job bound_service_account_names=default bound_service_account_namespaces=fieldops-data policies=read-all ttl=1h"
